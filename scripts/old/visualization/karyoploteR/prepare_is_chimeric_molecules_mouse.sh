...
# Script Goal: Prepare IS data file from chimeric molecules for karyoploteR package on MOUSE genome
# Author: Mallaury Vie
# Contributions by: 
# 
# Goal: 
...
while read sample; do
	...
	mkdir -p $workdir/visualization/karyoploteR/input/mouse/IS_chimeric_molecules/short

	# Give the file name of the AAV mapping BAM file filtered (no false positives)
	molecule_file="${workdir}/4.identify_chimeric_molecules/output/short/${sample}/${sample}_mapping_mouse.molecules.chimeric.bed"
	...
	awk '{print $1 "\t" $2 "\t" $3}' $molecule_file > $workdir/visualization/karyoploteR/input/mouse/IS_chimeric_molecules/short/${sample}_mouse_chimeric_IS.tmp

	# Rename the first column in 'chr'
	sed 1d $workdir/visualization/karyoploteR/input/mouse/IS_chimeric_molecules/short/${sample}_mouse_chimeric_IS.tmp | sed '1ichr\tstart\tend' > $workdir/visualization/karyoploteR/input/mouse/IS_chimeric_molecules/short/${sample}_mouse_chimeric_IS.bed

	# Clean
	rm $workdir/visualization/karyoploteR/input/mouse/IS_chimeric_molecules/short/${sample}_mouse_chimeric_IS.tmp

	# Launch R script "karyoploteR_mouse_chimeric_molecules_IS.R"


	#----------------- Extract chr, start and end positions of each IS identified for karyoploteR package --------------------#

	# Add a column to specify the length of each IS identified
