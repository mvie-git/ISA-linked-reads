...
workdir_data <- "/home/mallaury/Bureau/phd/project/GSDIa_louisa/ISA/visualization/karyoploteR/input/vector/IS_chimeric_molecules/"
...
sample <- list.files(path="/home/mallaury/Bureau/phd/project/GSDIa_louisa/ISA/visualization/karyoploteR/input/vector/IS_chimeric_molecules/", pattern="*_AAV_chimeric_IS.bed")
...
karyoploteR_IS_plot <- function(workdir, color, path_to_data, pos0, pos1) {
  ...
  # Add axis
  #kp.IS.axis <- kpAxis(kp.IS, ymin=0, ymax=max(IS.data$y), r0=pos0, r1=pos1, cex=0.7)
  
  # Add labels (number of total molecules per sample)
  kp.IS.sample.nb_molecules <- kpAddLabels(kp.IS, labels=nrow(data), r0=pos0, r1=pos1, data.panel=1, cex=0.9, col=color, side="right")
  
  return(kp.IS.sample.nb_molecules)
...
kp <- karyoploteR_plot(workdir_data, AAV_genome, AAV_cytobands)

# Add blacklist regions
## Add ymax
## TO DO! Change workdir directory!
kp <- karyoploteR_blacklist_regions_plot(workdir_blacklist, blacklist_regions[1], blacklist_regions[3], 1.5)

# Plot additional data: IS data for each sample
## Add regions for each sample
karyoploteR_IS_plot(workdir_data, color[1], sample[1], 0, 0.2) # 149
karyoploteR_IS_plot(workdir_data, color[4], sample[4], 0.25, 0.45) # 152
karyoploteR_IS_plot(workdir_data, color[2], sample[2], 0.50, 0.70) # 150
karyoploteR_IS_plot(workdir_data, color[3], sample[3], 0.75, 0.95) # 151
karyoploteR_IS_plot(workdir_data, color[5], sample[5], 1.0, 1.20) # 153
karyoploteR_IS_plot(workdir_data, color[9], sample[6], 1.25, 1.45) # 154
