...
while read sample; do
	...
	readID_list=${workdir}/7.select_readID_of_interest/${sample}/${sample}_readID_list.txt

	# Subset the index file keeping only barcodes assigned to the list of read ID of interest
	#LC_ALL=C grep -w -f $readID_list $index_file > ${workdir}/8.retrieve_barcodes_from_readID/${sample}/${sample}_subset_index_file.txt
	grep -w -f $readID_list $index_file > ${workdir}/8.retrieve_barcodes_from_readID/${sample}/${sample}_subset_index_file.txt

done < sample_list_8.txt
