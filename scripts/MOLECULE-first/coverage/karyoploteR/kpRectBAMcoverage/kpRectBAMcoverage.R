...
workdir <- "/media/mallaury/MALLAURY1/PROJECT/GSDIa_ISA/hybrid_genome_mapping/karyoploteR/kpRectBAMcoverage/"
AAV_genome <- "/media/mallaury/MALLAURY1/PROJECT/GSDIa_ISA/hybrid_genome_mapping/karyoploteR/genome/vector/AAV_vector_genome.txt"
AAV_cytobands <- "/media/mallaury/MALLAURY1/PROJECT/GSDIa_ISA/hybrid_genome_mapping/karyoploteR/genome/vector/AAV_vector_cytobands.txt"
sample <- list.files(path="/media/mallaury/MALLAURY1/PROJECT/GSDIa_ISA/hybrid_genome_mapping/karyoploteR/kpRectBAMcoverage/input/chimeric_molecules", pattern="*_chimeric.bed")
blacklist_regions <- list.files(path="/media/mallaury/MALLAURY1/PROJECT/GSDIa_ISA/hybrid_genome_mapping/karyoploteR/kpRectBAMcoverage/", pattern="vector_blacklist_regions_*")
...
karyoploteR_IS_plot <- function(workdir, color, path_to_data, pos0, pos1, sample_name) {
  ...
  IS.data <- toGRanges(data.frame(chr="chrVirus", start=data[, "start"], end=data[, "end"]))
...
kp <- karyoploteR_blacklist_regions_plot(workdir, blacklist_regions[1], blacklist_regions[2], 0.98)

# Plot additional data: IS data for each sample
## Add regions for each sample
karyoploteR_IS_plot(paste0(workdir, "input/chimeric_molecules/"), color[1], sample[2], 0, 0.20, "152") # 149
karyoploteR_IS_plot(paste0(workdir, "input/"), color[4], sample[1], 0.25, 0.45, "151") # 152
karyoploteR_IS_plot(paste0(workdir, "input/"), color[2], sample[3], 0.50, 0.70, "153") # 150
karyoploteR_IS_plot(paste0(workdir, "input/"), color[3], sample[4], 0.75, 0.95, "154") # 151
#karyoploteR_IS_plot(workdir, color[5], sample[5], 1.0, 1.20, "153") # 153
#karyoploteR_IS_plot(workdir, color[9], sample[6], 1.25, 1.45, "154") # 154
